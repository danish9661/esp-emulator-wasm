...
- 2026-09-19 update: StartAt pacing RULED OUT as the lever (StartAt is not hot — probe counts ~550 calls/3000 batches and hooking all nodes regresses 37 deterministically; B-only hook doesn't unblock 38). Time-dilation TESTED via `alarmNowPark` `tickDivShift` (srli knob, `loadFirmware(flash,elf,{thread:{tickDivShift}})`): div=1 halves B's OT clock (now=tick/2, retry gap ~750→~1500 batches) and div=2 quarters it, but B still never reaches role 2 (1500 rounds, roles [4]/[1,4]). Verdict: slowing B's clock alone does not stabilize the challenge — next hypotheses are OT-side stable-challenge retry (guest change: hold last challenge across retries until response or timeout) or harness-side challenge-aware gating (decrypt ParentReq via CTR-crib, §8, and suppress fresh-challenge retries while A's response to the current challenge is in flight).
...
- What remains: §9.1 StartAt shim RULED OUT (not hot; hook regresses, B-only doesn't unblock); §9.3 harness time-dilation TESTED via tickDivShift (div=1,2 slow cadence, still no attach — dilation alone insufficient); §9.2 OT-side stable-challenge retry (guest change, heavier) is now the prime candidate, followed by harness challenge-aware gating (CTR-crib decrypt, suppress stale-invalidating retries while A's response is in flight).
